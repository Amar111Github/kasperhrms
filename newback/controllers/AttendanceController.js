...
const findAttendance = async (req, res) => {
  try {
    ...
    const attendanceData = await AttendanceModel.find().populate('employeeObjID');
...
const todaysAttendance = async (req, res) => {
  try {
    ...
    const attendanceData = usersWithAttendance.map(user => {
      ...
      if (attendanceRecord && attendanceRecord.years && attendanceRecord.years.length > 0) {
        const yearData = attendanceRecord.years.find(year => year.year === currentYear);

        if (yearData && yearData.months && yearData.months.length > 0) {
          const monthData = yearData.months.find(month => month.month === currentMonth);
